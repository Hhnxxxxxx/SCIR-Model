function SIRpopulations()
    param.numvar = 3; % do not change
    ...
    param.beta  = 2.4*[1.00;1.00;1.00;1.00]./param.N;   % infection rates
    ...
    if shownetwork
        ...
        C = blkdiag(param.T_S,param.T_I,param.T_R);
    ...
    SIR0 = [param.N zeros(param.P,1) zeros(param.P,1)];
    SIR0(end,:) = param.N(end) * [0.95,0.05,0];

    % compute the (initial) basic reproductive number for each population 
    R0 = param.beta .* SIR0(:,1) ./ param.gamma;
    ...
    if any(abs( sum(S+I+R,2) - sum(param.N) ) > 1e-8)
    ...
    labs = cell(param.numvar*param.P,1);
    clab = {'S','I','R'};
    for i=1:param.P
        for c=1:param.numvar
...
% function that computes the right-hand side of the differential equation
    X = reshape(x,param.P,param.numvar);

    S = X(:,1); I = X(:,2); R = X(:,3); % extract the variables
    
    dxdt = [ -param.beta  .* I .* S                    + param.T_S*S - sum(param.T_S,1)'.*S, ... dS/dt
              param.beta  .* I .* S - param.gamma .* I + param.T_I*I - sum(param.T_I,1)'.*I, ... dI/dt
              param.gamma .* I                         + param.T_R*R - sum(param.T_R,1)'.*R  ... dR/dt
           ];
